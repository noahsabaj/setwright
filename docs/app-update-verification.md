...
The macOS fixture later stalled on its XPC request path. The existing reviewed deadline implementation was brought over from the local PDF compatibility branch: independent connections for control and waiting, bounded broker calls, a service wall-clock limit, and orphan cleanup. This preserves code-signing requirements and App Sandbox containment; successful hosted execution is required before merge.

The process-count probe now starts at most one child beyond the 32-child policy, records the reached threshold, and requires evidence of rejection or termination after reaching that threshold. This avoids exhausting the CI host and prevents an unrelated early failure from masquerading as a passing process-limit check. Linux accounting includes bubblewrap's two infrastructure processes separately from the compiler's child allowance.
